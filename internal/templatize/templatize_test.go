...
func TestTemplateSQL_empty(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()

	template, params, err := parser.TemplatizeSQL("")
...
func TestTemplateSQL_between_and(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
	...
	as.Equal(nil, err)
	as.Equal(
		"SELECT * FROM users WHERE create_time BETWEEN ? AND ?",
		template,
	)
	as.Equal(2, len(params))
}

func TestTemplateSQL_in(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
	...
	as.Equal(nil, err)
	as.Equal(
		"SELECT * FROM users WHERE name eq ? and uuid NOT IN (?, ?)",
		template,
	)
	as.Equal(3, len(params))
...
func TestTemplateSQL_like(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
...
func TestTemplateSQL_GroupBy(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
	...
	as.Equal(4, len(params))
}

func TestTemplateSQL_OrderBy(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
...
func TestTemplateSQL_AggregateFunc_AS(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
...
func TestTemplateSQL_Limit(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
...
func TestTemplateSQL_Having(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
...
func TestTemplatizeSQL_Insert(t *testing.T) {
	...
	template, params, err = NewSQLTemplatizer().TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("INSERT INTO users VALUES (?, ?, ?, ?, ?, ?)", template)
	as.Equal(6, len(params))
...
func TestTemplatizeSQL_Update(t *testing.T) {
	...
	// UPDATE with subquery
	sql = "UPDATE users SET name = 'Alice', age = (SELECT age FROM users WHERE name = 'Alice'), high = 173, weight = 150, level = (SELECT level FROM users WHERE name = 'Alice'), create_time = '2021-01-01 00:00:00' WHERE id = (SELECT id FROM users WHERE name = 'Alice') ORDER BY name, age LIMIT 20"
	template, params, err = NewSQLTemplatizer().TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal(
		"UPDATE users SET name eq ?, age eq (SELECT age FROM users WHERE name eq ?), high eq ?, weight eq ?, level eq (SELECT level FROM users WHERE name eq ?), create_time eq ? WHERE id eq (SELECT id FROM users WHERE name eq ?) ORDER BY name, age LIMIT ?",
		template,
	)
	as.Equal(8, len(params))
...
func TestTemplatizeSQL_ComplexUpdate(t *testing.T) {
	...
	template, params, err := paser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("UPDATE users AS u1 CROSS JOIN users AS u2 SET u1.name eq ?, u1.age eq ?, u1.high eq ?, u1.weight eq ?, u2.level eq ?, u2.create_time eq ?", template)
	as.Equal(6, len(params))
...
func TestTemplatizeSQL_case_when(t *testing.T) {
	...
	template, params, err := paser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("UPDATE users SET name eq CASE id WHEN ? THEN ? ELSE ? END, age eq CASE id WHEN ? THEN ? ELSE ? END WHERE id eq ?", template)
	as.Equal(7, len(params))
	...
	template, params, err = paser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("UPDATE users SET name eq CASE WHEN id eq ? THEN ? WHEN id eq ? THEN ? ELSE ? END WHERE id lt ?", template)
	as.Equal(6, len(params))
}

func TestTemplateSQL_Delete(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	...
	template, params, err := NewSQLTemplatizer().TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("DELETE FROM users WHERE id eq ?", template)
	as.Equal(1, len(params))
	...
	template, params, err = NewSQLTemplatizer().TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("DELETE FROM users", template)
	as.Equal(0, len(params))
	...
	template, params, err = NewSQLTemplatizer().TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("DELETE FROM users WHERE id eq ? LIMIT ?", template)
	as.Equal(2, len(params))
	...
	template, params, err = NewSQLTemplatizer().TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("DELETE FROM users ORDER BY name, age LIMIT ?", template)
	as.Equal(1, len(params))
...
func TestTemplateSQL_complex(t *testing.T) {
	t.Parallel()
	as := assert.New(t)

	// UPDATE with multiple tables
	sql := "INSERT INTO tb6 (`sKey`,`sBody`,`dtCreateTime`,`iAppId`,`sModule`,`iActId`,`sUid`,`sBizCode`,`iVersion`,`sAction`) VALUES ('order_LZJD-20241115151702783-560116327_2','','2024-11-26 21:23:07','1001','ConfirmTradi','22450','481286327','lzjd','1','{\"default_ip\":\"\",\"l5_cmd\":\"1703936\",\"l5_mod\":\"64210497\",\"nobody\":\"1\",\"times\":\"0\",\"url\":\"http://api.ams.ied.tencent-cloud.net/daoju/go/iapi/red_dot?opt_type=decr&uid=481286327&_biz_code=lzjd&red_type=1&_t=1731655024\"}');"

	template, params, err := NewSQLTemplatizer().TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("INSERT INTO tb6 (sKey, sBody, dtCreateTime, iAppId, sModule, iActId, sUid, sBizCode, iVersion, sAction) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", template)
	as.Equal(10, len(params))
}

func TestTemplateSQL_MultipleStatements(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	psr := NewSQLTemplatizer()
	...
	`
	template, params, err := psr.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal(
		"INSERT INTO users (name, age) VALUES (?, ?); UPDATE users SET age eq ? WHERE name eq ?; DELETE FROM users WHERE name eq ? and age gt ?",
		template,
	)
	as.Equal(6, len(params))
	as.Equal("Alice", params[0])
	as.Equal(int64(25), params[1])
	...
	`
	_, _, err = psr.TemplatizeSQL(sql)
	as.Error(err)

	// Test error case with invalid SQL at the end
	sql = "INSERT INTO tbTradiQueueRT_6 (`sKey`,`sBody`,`dtCreateTime`,`iAppId`,`sModule`,`iActId`,`sUid`,`sBizCode`,`iVersion`,`sAction`) VALUES ('order_LZJD-20241115151702783-560116327_2','','2024-11-26 21:23:07','1001','ConfirmTradi','22450','481286327','lzjd','1','{\"default_ip\":\"\",\"l5_cmd\":\"1703936\",\"l5_mod\":\"64210497\",\"nobody\":\"1\",\"times\":\"0\",\"url\":\"http://api.ams.ied.tencent-cloud.net/daoju/go/iapi/red_dot?opt_type=decr&uid=481286327&_biz_code=lzjd&red_type=1&_t=1731655024\"}');INSERT INTO tbTradiQueueUK (`dtCreateTime`,`iActId`,`iVersion`,`sBody`,`sModule`,`iAppId`,`sAction`,`sBizCode`,`sKey`,`sUid`) VALUES ('2024-11-26 21:23:07','22450','1','','ConfirmTradi','1001','[{\"default_ip\":\"\",\"l5_cmd\":\"1703936\",\"l5_mod\":\"64210497\",\"nobody\":\"1\",\"times\":\"0\",\"url\":\"http://api.ams.ied.tencent-cloud.net/daoju/go/iapi/red_dot?opt_type=decr\\u0026uid=481286327\\u0026_biz_code=lzjd\\u0026red_type=1\\u0026_t=1731655024\"}]','lzjd','order_LZJD-20241115151702783-560116327_2','481286327')"
	template, params, err = psr.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("INSERT INTO tbTradiQueueRT_6 (sKey, sBody, dtCreateTime, iAppId, sModule, iActId, sUid, sBizCode, iVersion, sAction) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?); INSERT INTO tbTradiQueueUK (dtCreateTime, iActId, iVersion, sBody, sModule, iAppId, sAction, sBizCode, sKey, sUid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", template)
	as.Equal(20, len(params))
}

func TestTemplateSQL_Parentheses(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
	...
	template, params, err := parser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("SELECT * FROM users WHERE (age gt ?)", template)
	as.Equal(1, len(params))
	...
	template, params, err = parser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("SELECT * FROM users WHERE (age gt ? and (height gt ? or weight lt ?))", template)
	as.Equal(3, len(params))
	...
	template, params, err = parser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("SELECT * FROM users WHERE (id IN (?, ?, ?) or (age gt ? and height gt ?))", template)
	as.Equal(5, len(params))
	...
	template, params, err = parser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("SELECT *, (price mul quantity) AS total FROM orders WHERE (price mul quantity) gt ?", template)
	as.Equal(1, len(params))
	...
	template, params, err = parser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("SELECT * FROM users WHERE (CASE WHEN age gt ? THEN ? ELSE ? END) eq ?", template)
	as.Equal(4, len(params))
	...
	template, params, err = parser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("SELECT *, (COUNT(1) plus SUM(quantity)) AS total FROM orders GROUP BY user_id HAVING (COUNT(1) plus SUM(quantity)) gt ?", template)
	as.Equal(1, len(params))
	...
	template, params, err = parser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("SELECT (COUNT(1) plus sum(2.0) plus avg(3.0)) AS total FROM orders", template)
	as.Equal(0, len(params))
}

func TestTemplateSQL_FuncCall(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
...
func TestTemplateSQL_UnaryOperation(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
	...
	as.Equal(nil, err)
	as.Equal(
		"SELECT minus age, minus ? FROM users WHERE score gt minus ?",
		template,
	)
	as.Equal(2, len(params))
	...
	as.Equal(nil, err)
	as.Equal(
		"SELECT * FROM users WHERE not is_deleted and not (age lt ? or level gt ?)",
		template,
	)
	as.Equal(2, len(params))
	...
	as.Equal(nil, err)
	as.Equal(
		"SELECT bitneg flags FROM users WHERE bitneg permission_bits eq ?",
		template,
	)
	as.Equal(1, len(params))
}

func TestTemplateSQL_IsNull(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
	...
	as.Equal(nil, err)
	as.Equal(
		"SELECT * FROM users WHERE email IS NOT NULL and last_login IS NOT NULL",
		template,
	)
	as.Equal(0, len(params))
	...
	as.Equal(nil, err)
	as.Equal(
		"SELECT * FROM users WHERE email IS NULL and age gt ? and status IS NOT NULL",
		template,
	)
	as.Equal(1, len(params))
}

func TestTemplateSQL_Exists(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
...
func TestTemplateSQL_Default(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
	...
	as.Equal(nil, err)
	as.Equal(
		"INSERT INTO users (name, created_at) VALUES (?, DEFAULT)",
		template,
	)
	as.Equal(1, len(params))
	as.Equal("Alice", params[0])
	...
	as.Equal(nil, err)
	as.Equal(
		"INSERT INTO users (name, age) VALUES (DEFAULT name, DEFAULT age)",
		template,
	)
	as.Equal(0, len(params))
	...
	as.Equal(nil, err)
	as.Equal(
		"INSERT INTO users (name, age) VALUES (?, ?), (DEFAULT, DEFAULT)",
		template,
	)
	as.Equal(2, len(params))
	as.Equal("Alice", params[0])
	...
	as.Equal(nil, err)
	as.Equal(
		"INSERT INTO users (name, age, created_at) VALUES (DEFAULT, ?, DEFAULT)",
		template,
	)
	as.Equal(1, len(params))
	as.Equal(int64(26), params[0])
}

func TestTemplateSQL_TimeUnit(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
	...
	as.Nil(err)
	as.Equal(
		"SELECT * FROM orders WHERE create_time gt DATE_SUB(NOW(), INTERVAL ? DAY)",
		template)
	as.Equal(1, len(params))
	...
	as.Nil(err)
	as.Equal(
		"SELECT * FROM events WHERE event_time lt DATE_ADD(NOW(), INTERVAL ? HOUR)",
		template)
	as.Equal(1, len(params))
	...
	as.Nil(err)
	as.Equal(
		"UPDATE tasks SET due_date eq DATE_ADD(created_at, INTERVAL ? MINUTE)",
		template)
	as.Equal(1, len(params))
	...
	as.Nil(err)
	as.Equal(
		"SELECT DATE_ADD(start_date, INTERVAL ? MONTH) AS end_date FROM projects",
		template)
	as.Equal(1, len(params))
	...
	as.Nil(err)
	as.Equal(
		"SELECT * FROM employees WHERE hire_date lt DATE_SUB(NOW(), INTERVAL ? YEAR)",
		template)
	as.Equal(1, len(params))
}

func TestTemplateSQL_Explain(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
	...
	template, params, err := parser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("EXPLAIN FORMAT = row SELECT * FROM users WHERE id eq ?", template)
	as.Equal(1, len(params))
	...
	template, params, err = parser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("EXPLAIN ANALYZE FORMAT = row SELECT * FROM users WHERE name eq ? and age gt ?", template)
	as.Equal(2, len(params))
	...
	template, params, err = parser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("EXPLAIN FORMAT = JSON SELECT * FROM users WHERE id IN (?, ?, ?)", template)
	as.Equal(3, len(params))
	...
	template, params, err = parser.TemplatizeSQL(sql)
	as.Equal(nil, err)
	as.Equal("EXPLAIN ANALYZE FORMAT = JSON SELECT u.* FROM users AS u CROSS JOIN orders AS o ON u.id eq o.user_id WHERE o.status eq ?", template)
	as.Equal(1, len(params))
}

func TestTemplateSQL_InvalidSQL(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
...
func TestTemplateSQL_CrossJoin(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
...
func TestTemplateSQL_RightJoin(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
	...
	template, params, err := parser.TemplatizeSQL(sql)
	as.Nil(err)
	as.Equal("SELECT * FROM users RIGHT JOIN orders ON users.id eq orders.user_id", template)
	as.Equal(0, len(params))
}

func TestTemplateSQL_UnaryOperations(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
	...
	template, params, err := parser.TemplatizeSQL(sql)
	as.Nil(err)
	as.Equal("SELECT * FROM users WHERE not (age gt ?)", template)
	as.Equal(1, len(params))
	...
	template, params, err = parser.TemplatizeSQL(sql)
	as.Nil(err)
	as.Equal("SELECT * FROM users WHERE balance lt minus ?", template)
	as.Equal(1, len(params))
}

func TestTemplateSQL_MultipleErrors(t *testing.T) {
	t.Parallel()
	as := assert.New(t)
	parser := NewSQLTemplatizer()
